---
title: "New Orleans Analysis - September 11, 2018"
output: pdf_document
---
...
Load the set of predictions for four mile square covering the New Orleans Spillway generated using a clustered Naive Bayes classifier with training on the set of images in "pothole_training_image.tif" using the bands c(4,5,6). 

```{r}
df <- read.csv("~/GEE/data/new_orleans/new_orleans_predictions_0.3_CSSM.csv")
...
ggplot(df) + 
  geom_line(aes(x = doy, y = pred*9/10, col=as.factor(year)), size = 0.5) + 
  ylab("Water extent (1000 m^2)") +
  xlab("DOY") +
  theme(text = element_text(size=15))
...
#ggsave(timeseries, filename = "new_orleans_timeseries_full_data.tiff", dpi = 1200, width = 8.5, height = 5, units = "in")
```
...
```{r}
plot(x = (df %>% filter(cloud < 101))$dates, y = (df %>% filter(cloud < 101))$pred, "p", xlab = "Year", ylab = "Number of pixels classifies as water", pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[1], main="Classification of water over time after imputation")
points(x = (df %>% filter(cloud < 100))$dates, y = (df %>% filter(cloud < 100))$pred, pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[2])
points(x = (df %>% filter(cloud < 75))$dates, y = (df %>% filter(cloud < 75))$pred, pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[3])
points(x = (df %>% filter(cloud < 50))$dates, y = (df %>% filter(cloud < 50))$pred, pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[4])
points(x = (df %>% filter(cloud < 25))$dates, y = (df %>% filter(cloud < 25))$pred, pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[5])
points(x = (df %>% filter(cloud < 10))$dates, y = (df %>% filter(cloud < 10))$pred, pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[6])
points(x = (df %>% filter(cloud < 1))$dates, y = (df %>% filter(cloud < 1))$pred, pch = 19, col = RColorBrewer::brewer.pal(7, "Dark2")[7])
lines(x = (df %>% filter(cloud < 1))$dates, y = (df %>% filter(cloud < 1))$pred, "l")
legend(col = RColorBrewer::brewer.pal(7, "Dark2"), legend = c("= 100", "< 100", "< 75", "< 50", "< 25", "< 10", "< 1"), "topleft", pch = 19, title = "Percent imputed")
```
...
```{r}
images <- brick("~/GEE/data/new_orleans/new_orleans_images.tif")
images <- dropLayer(images, 1)
```

```{r}
aniBrick_no <- brick("~/GEE/data/new_orleans/new_orleans_raster_0.3_CSSM.tif")
...
```{r}
tiff(filename = "new_orleans_spatial_full_data.tiff", width = 8.5, height = 8.5, units = "in", res = 1200)
...
```{r}
t <- read.csv("~/GEE/data/new_orleans/outlier_analysis_no.csv")
brick <- aniBrick_no
...
ggplot2::ggplot(data = t %>% mutate(quantile = ntile(fluctuation_water_no, 10)) %>% group_by(quantile) %>% summarise(mean = mean(fluctuation_water_no), sd = sd(fluctuation_water_no)), aes(x = log(mean), y = log(sd))) + ggplot2::geom_point() + ggplot2::geom_smooth(method = "lm")

ggplot2::ggplot(data = t %>% mutate(quantile = ntile(fluctuation_land_no, 10)) %>% group_by(quantile) %>% summarise(mean = mean(fluctuation_land_no), sd = sd(fluctuation_land_no)), aes(x = log(mean), y = log(sd))) + ggplot2::geom_point() + ggplot2::geom_smooth(method = "lm")
...
#for(i in 1:(length(df$row))){plot(land_brick_no[[i]], main = df$row[i], col = rainbow(50)[35:50]);}
...
ggsave(timeseries2, filename = "new_orleans_timeseries_outliers_rm.tiff", dpi = 1200, width = 16.5, height = 8, units = "cm", device = "tiff")
...
#ggsave(timeseries3, filename = "new_orleans_timeseries_all.eps", dpi = 1200, width = 8.5, height = 8.5, units = "in")
```
